...
import { getColumns, getQuotationsColumns, getWareHouseColumns, getWidgetColumns } from '@/app/shared/ecommerce/order/order-list/columns';
import { ActionIcon } from 'rizzui';
import cn from '@/utils/class-names';
import ExpandedOrderRow, { ExpandedOrderRowWareHouse } from '@/app/shared/ecommerce/order/order-list/expanded-row';
import { getAgentQuotations, getAllAgents, getAllQuotations, getApprovedQuotations, getCustomerQuotations } from '@/data/order-data';
...
export const OrderTableForWareHouse = ({
  data,
  variant = 'modern',
  className,
}: {
  ...
  const getQuotationsList = async () => {
    ...
    if(session.data?.userData.userrole === 'buyer'){
      ...
      const data = await getApprovedQuotations();
      const agents = await getAllAgents();
      setAllAgents(agents)
      setQuotations(data)
      setIsLoading(false);
  ...
  const columns = useMemo(
    () => getWareHouseColumns({ sortConfig, onHeaderCellClick, onDeleteItem }),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [onHeaderCellClick, sortConfig.key, sortConfig.direction, onDeleteItem]
  );
  ...
  return (
    <div className={cn(className)}>
      <ControlledTable
        ...
        expandable={{
          expandIcon: CustomExpandIcon,
          expandedRowRender: (record) => <ExpandedOrderRowWareHouse record={record} agents={allAgents} reloadFunction={getQuotationsList} />,
        }}
        // paginatorOptions={{
        //   pageSize,
        //   setPageSize,
